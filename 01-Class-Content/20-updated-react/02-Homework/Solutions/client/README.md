...
in all web browsers. Service worker registration won't be attempted in `./src/registerServiceWorker.js`
...
option in the `SWPrecacheWebpackPlugin` section of `config.prod.js`
...
options of the `SWPreachePlugin configuration` in `../config/webpack.config.prod.js`.
