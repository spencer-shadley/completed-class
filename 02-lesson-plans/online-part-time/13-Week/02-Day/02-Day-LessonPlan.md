...
* This lesson's slides are available on Google Drive here: [13-2 Express Yourself](https://docs.google.com/presentation/d/1X9FIvr_yxp_DRticllPkUAvZYsjut2iJK7P7EKMLdy4/edit?usp=sharing)
...
* Explain that `express.json` and `express.urlEncoded` are examples of **middleware**.
  ...
  * Middleware can transform the request from the browser before we work with it. In the case of `express.json` and `express.urlEncoded`, they take the unreadable request and turn it into a readable object and attaches it to `req.body`. By the time the request gets to our routes, our body parsing middleware has already formatted it for us.
