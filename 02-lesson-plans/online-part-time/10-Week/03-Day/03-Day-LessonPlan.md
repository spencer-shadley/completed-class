...
* Open the folder `23-GeocodeNPM` in your terminal. Be sure to replace `YOUR-MAPQUEST-API-CONSUMER-KEY` in `geocodeChallenge.js` with your MapQuest API key (if you haven't already signed up for one, do so [here](https://developer.mapquest.com/plan_purchase/steps/business_edition/business_edition_free/register)). Avoid demonstrating the code to students.

* Run `node geocodeChallenge.js Austin Texas` and demonstrate to students that the code `console.log`s a JSON object that geocodes the location. Explain that this information is very useful when building map-based applications.
...
* **Instructions:**
  ...
  * You can choose to make one of two assumptions when completing this application.
    ...
    * Harder Option: The user can provide a location in any format. In fact, they can even provide a landmark instead of an address. ("151 Sip Ave, Jersey City NJ", "1600 Pennsylvania Avenue NW Washington, D.C", etc.)
...
* In discussing the solution be sure to point out each of the following:
  ...
  * How we finally passed the address into the `node-geocoder` function and `console.log`'d the data it returned.

  ![/04-GeocoderNPM_1.png](Images/04-GeocoderNPM_1.png)

* Then open the file `geocodeChallenge` (`23-GeocodeNPM > SOLVED`)

* Let students know that the solution is very similar except for one major difference. In this case, we can't be sure how many words long the address location will be. So instead, we need to create a for-loop that flips through each of the words in the user's location and assembles the `address` from each word. We can then pass the address in the same way, thus allowing the `node-geocoder` package to handle the heavy lifting of determining if an address is a city, zip code, etc.

  ![/04-GeocoderNPM_2.png](Images/04-GeocoderNPM_2.png)

* See if there are any questions before proceeding to the next example.

### 10.	Instructor Do: Send 24-PackageJSON.zip	(1 mins)
...
* After fielding a few responses, run the code yourself using `node weatherDest.js Los Angeles, California`. (Don't forget to include a location!)
...
* Now for the _really important part_.
  ...
  * Delete both the `package.json` file and the  `node_modules` folder from `24-PackageJSON`.
