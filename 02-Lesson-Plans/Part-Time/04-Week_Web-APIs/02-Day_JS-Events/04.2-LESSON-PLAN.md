# 4.2 Lesson Plan - Events Extravaganza (6:30 PM) <!--links--> &nbsp; [⬅️](../01-Day/01-LESSON-PLAN.md) &nbsp; [➡️](../03-Day/03-LESSON-PLAN.md)
...
* Ask the class the following question(s):
  ...
  * ☝️ How would we build this application?
...
* Ask the class the following question(s):
  ...
  * ☝️ What are the first things our app needs so a user can add another user to the list?

  * 🙋 We need an input field for `name` and we need some way to gather the data from `name` when `Add Person` is pressed.

  * ☝️ When a user adds a new user to the HTML form and clicks `Add Person`, what happens next?

  * 🙋 We need to dynamically create a new list item, then append an `edit` button to that new list item. Lastly, we append the new list item to the main people list.

  * ☝️ What needs to happen when a user clicks the "edit" button?
  ...
  * ☝️ What needs to happen when a user clicks the `save` button?"

  * 🙋 The modal needs to disappear and we need some way of keeping track of which person has which description.

  * ☝️ How can we make our web applications more interactive?
...
* Ask the class the following question(s):
  ...
  * ☝️ What do you remember about forms?

  * 🙋 There are several type of inputs including check boxes, text fields, radio buttons, and select. Forms also have default behaviors that occur on submission.
...
* Ask the class the following question(s) and call on students for the corresponding answer(s):
    ...
    * ☝️ Why might this be a problem?
  
    * 🙋 * We may want to retain that information on the page after the form has been submitted or we could have other buttons within the form that should not submit the results.
...
* Ask the class the following question(s) and call on students for the corresponding answer(s):
  ...
  * ☝️ What are these properties are used for?"
