...
* Ask the students the following question(s):
  ...
  * ☝ If our application returns a string, what do we think the code looks like?
...
* Use the following prompts to review these key points:
  ...
  * The `node` command allows us to run JavaScript, Node.js, from the command line.

  * `global` is a reference to the Node module that invokes it
...
* Use the following prompts to demonstrate the key point(s): 
...
* Ask the class, 'What is different about this example?'
...
* Ask the class, 'How would we extend the functionality of Node?'
...
* Ask the class, 'Do we see a solution to our missing module problem?'
...
* Use the following prompts to explain these key points: 
