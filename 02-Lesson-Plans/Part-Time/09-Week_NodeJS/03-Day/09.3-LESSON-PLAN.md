...
* Describe approaches for handling asynchronous behavior with JavaScript
...
* Working with asynchronous code can be more confusing that working with synchronous code, but much of the most essential functionality in web applications happens asynchronously.

* Remind the class that since Node is single threaded, we need to write asynchronous code effectively in order to avoid blocking client requests on the server.

* Explain that we'll conclude this lesson with a mini project where students will build a portfolio generating command line tool.
...
* Open the [Solved version of the activity](../../../../01-Class-Content/09-NodeJS/01-Activities/31-Stu_Callback-Review/Solved) in your editor.
...
* From the solution code, demonstrate the following key point(s):
  ...
  * ✔️ Since any data read with `fs.readFile` comes back as a string, we're using the `JSON.parse` method to turn the JSON string into a JSON object we can work with.
...
* Explain that callback functions themselves aren't asynchronous any more than regular named functions are.

* Callback functions _are_ often used to describe behavior that should happen at some later time. But in the case of `forEach`, the provided function is run once for each element in the array in a synchronous loop. The callback function is run right away, there's nothing to wait for.
...
* Demonstrate the following key point(s):
  ...
  * ✔️ After axios is used to perform an AJAX request to the URL, an array of repo names is created from the response data using the `map` method.
...
* We've been using Promises for a while now -- since learning jQuery -- without really talking about them. Any time we wrote code that used `.then`, we were working with a Promise object.
...
* Open the [35-Stu-Joke-Saver/Solved/index.js](../../../../01-Class-Content/09-NodeJS/01-Activities/35-Stu-Joke-Saver/Basic/Solved) in your editor:
...
* So far we aren't doing anything with Promises that we couldn't do with callbacks alone. One of the advantages of using Promises starts to become clear in the solution to the bonus activity.

* Open [35-Stu-Joke-Saver/Solved/Bonus/index.js](../../../../01-Class-Content/09-NodeJS/01-Activities/35-Stu-Joke-Saver/Solved/Bonus) and demonstrate the code:
...
* Open [36-Ins_Promise-Constructor](../../../../01-Class-Content/09-NodeJS/01-Activities/36-Ins_Promise-Constructor) in your editor:
...
* Demonstrate the following key key point(s):
...
* Inform the class that they shouldn't expect to have to create many of their own Promises in the near future, but having a high-level understanding of how that's done will make them more effective using third-party Promise-based libraries.
...
* Open [39-Stu_Async-Await/Solved/Bonus/index.js](../../../../01-Class-Content/09-NodeJS/01-Activities/39-Stu_Async-Await/Solved/Bonus) in your editor:
...
* Answer any remaining questions before dismissing the class for break.
