...
* Ask the class the following question(s): 
  ...
  * 🙋 Two and the first one is the DB name, the second is the version number.
...
* Ask the class the following question(s):
  ...
  * 🙋 The call to the open() function returns a request object with a `result` (success) value that you handle as an event. 
...
* Use the prompts and talking points below to demonstrate the following key point(s):
  ...
  * ✔ We create our object stores in the `onupgradeneeded` method.
...
* Use the prompts and talking points below to demonstrate the following key point(s):
  ...
  * ✔ We use the `createIndex(indexName, keyPath)` function to create indexes, it takes two arguments.
...
* Open [23-Stu_Creating_Indexes](../../../../01-Class-Content/17-nosql/01-Activities/23-Stu_Creating_Indexes/Solved/index.html) in your IDE and explain the following point(s):
  ...
  * We simply call the `createIndex` method on our object store and create tree new indexes, giving each of them an `indexName` and `keyPath`.
