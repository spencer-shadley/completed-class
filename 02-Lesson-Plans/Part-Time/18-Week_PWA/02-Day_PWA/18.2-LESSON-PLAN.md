# 18.2 Lesson Plan - Progressive Web Applications (6:30 PM) <!--links--> &nbsp; [⬅️](../01-Day/01-Day-LessonPlan.m) &nbsp; [➡️](../01-Day/01-Day-LessonPlan.md)
...
* Let the students know that each step can be found in the instructions located in [08-Stu_PWAs](../../../../01-Class-Content/18-PWA/01-Activities/07-Stu_PWAs)
...
* Use the prompts and talking points below to demonstrate the following key point(s):
  ...
  * A service worker is a script that your browser runs in the background on a separate thread from your webpage.
...
* Run [10-Ins_Service_Workers](../../../../01-Class-Content/18-PWA/01-Activities/10-Ins_Service_Workers) and ensure that `mongod` is running.
...
* Use the prompts and talking points below to review the following key point(s):
  ...
  * The main takeaway is that we are adding an event listener to our window element, registering our service worker and then logging a simple message to let us know it was successful.

  * 🎗 Remember that it is not necessary that we understand every single line of code at this point, but that we have a general understanding of we we are doing. This code will make more sense as we get used to working with service workers.
...
* Visit <http://localhost:3000> and open your Chrome Dev Tools > Sources.
  ...
  * In the Sources tab, our service worker is running on a separate thread.
...
* Open [12-Ins_Caching_Fetching_Files/Solved/public/service-worker.js](../../../../01-Class-Content/18-PWA/01-Activities/12-Ins_Caching_Fetching_Files/Solved/public/service-worker.js) in your IDE and explain the following: 
  ...
  * We will be adding this code to our `service-worker.js` file in the next activity, but first we want to gain a high level understanding of what is happening.
...
* Open [12-Ins_Caching_Fetching_Files/public/assets/js/loadPosts.js](../../../../01-Class-Content/18-PWA/01-Activities/12-Stu_Caching_Fetching_Files/public/assets/js/loadPosts.js) in your IDE and explain the following: 
...
* There is quite a bit of code here so take the time to step through it, clarifying any questions as you go.
  ...
  * 🎗Remember to focus on the big picture of creating an offline experience and the details will fall into place. 
...
* Direct students to the activity instructions found in [16-Stu_Notetaker_PWA](../../../../01-Class-Content/18-PWA/01-Activities/16-Stu_Notetaker_PWA)
...
* Open [16-Stu_Notetaker_PWA/Solved/public/manifest.webmanifest](../../../../01-Class-Content/18-PWA/01-Activities/16-Stu_Notetaker_PWA/Solved/public/manifest.webmanifest) in your IDE and explain the following: 
...
* Open [16-Stu_Notetaker_PWA/Solved/public/service-worker.js](../../../../01-Class-Content/18-pwa/01-Activities/16-Stu_Notetaker_PWA/Solved/public/service-worker.js) in your IDE and explain the following: 
...
* Congratulate the students for making through the PWA content. 

  * Not only were the materials covered bleeding edge, but we worked with much larger codebases than we are used to. This serves as preparation for future careers in which you will be expected to navigate through large codebases that contain lots of code that you didn't write. 

  * Again, don't worry if there were bits of code that you didn't understand. The focus of today was to learn about why we use service workers, and the steps that are necessary to create and install a PWA.
