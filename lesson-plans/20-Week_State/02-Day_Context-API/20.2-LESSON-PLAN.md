...
- The demos and activities will only include the `src` directory which you will need to replace in your React app's boilerplate to avoid repetitive installs. It's recommended that you completely restart the dev server between activities.
...
- Run [useContext Demo] by copying the `src` directory into your prepared CRA application. Navigate to http://localhost:3000/ in your browser and demonstrate the following:
...
### Stu: Hooking in Context Activity
...
- Open [Content.js Solved] and point out the following:
...
- Update this application to include the following:
  ...
  - Add a Context API file to the `utils` directory.
