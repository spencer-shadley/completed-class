...
- Replace your React app's `src` directory with [10-Ins_PropsDemo/src]. Stop the dev server if it is already running. Start the app in dev mode by running `npm start`.
...
- Replace your Create React App's `src` directory with [14-Ins_BasicState]. Stop the dev server if it is already running. Start the app in dev mode by running `npm start`.
