...
- All of the Solved/Unsolved activities contain only the application's `src` directory. At the start of class scaffold out a React application using Create React App, and swap out the `src` directory for each activity to avoid repetitive npm installs. It's recommended that you completely restart the dev server between activities.
...
### Ins: Go through Slides (10 min) (High)
...
- Replace your React app's `src` directory with [10-Ins_PropsDemo/src]. Stop the dev server if it is already running. Start the app in dev mode by running `npm run watch`.
...
### Partners: Props Review (15 min) (Critical)
...
- In your editor's sidebar, point out how each component is contained inside of its own directory containing a CSS file, and an `index.js` file.

  - Ask the class: "Why are we using `index.js` to hold the component instead of `FriendCard.js`?"

  - Whenever we require/import a directory instead of a file, the directory's `index.js` file is required/imported by default (if it exists).
  ...
  - Giving all of our components their own directory is another option for organizing our React apps. Each directory could contain any CSS or other dependencies the component will need.
...
### Stu: Component Map (10 min) (Critical)
...
### Ins: Introduce Stateful Components
...
- Replace your Create React App's `src` directory with [14-Ins_BasicState]. Stop the dev server if it is already running. Start the app in dev mode by running `npm run watch`.
...
- ### Stu: Friend Refactor (20 min) (Critical)
