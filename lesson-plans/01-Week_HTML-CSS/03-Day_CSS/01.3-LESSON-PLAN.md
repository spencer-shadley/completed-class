...
### Ins: Welcome + Admin Tasks (Slides) (3 min)
...
### Ins: Warning Monologue + Student Questions (Slides)
...
### Ins: Relative File Paths Slides + Demo
...
- First open the HTML file `html-bio-with-css.html` that is inside of the directory `07-RelativePaths-Activity/RelativePaths_DEMO` in your browser. Show students what the styled site looks like.
...
- Point out to students again that the href needs to be able to "see" the CSS file and that href is like our "map" to where the file is.
  ...
  - Then move the CSS file into a directory called `assets`. Refresh the page in the browser and point out that the HTML page no longer knows where the CSS file is.
...
### Stu: Relative File Paths Activity (12 min)
...
- Slack out this file path to students (they should `git pull` and navigate there):
  ...
  - Modify each of the four `html-bio.html` pages such that they can access the CSS inside their directory. Don't move the CSS file, and don't move the HTML file.
...
- Review each of the Relative File Path examples. You can use the `RelativePaths_SOLVED` directory to quickly show solutions if you prefer.

### Ins: Box Model (Slides)
...
### Partners: Float Activity (30 min)
...
### Ins: CSS Positioning Slides + Demo
...
- Show students both the CSS used and the impact on the HTML.
  ...
  - Slack students the entire directory (zipped) when done with this activity.

### Partners: CSS Positioning Activity (30 min)
