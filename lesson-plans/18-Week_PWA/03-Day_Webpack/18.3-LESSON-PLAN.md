...
- Open `webpack.config.js` in your IDE and point out the following:
  ...
  - Output is an object describing the bundle that Webpack will build. The path property is the directory that the file will be created in. In this configuration, we are telling Webpack that the output file should be in a directory named `dist` and the file itself should be named `bundle.js`.
...
- Run `webpack` from the command line and explain the following:
  ...
  - Running `webpack` created a new directory named `dist` with the new bundle inside a file named `bundle.js`.
...
# 4. Instructor Do: Review Budget Tracker
