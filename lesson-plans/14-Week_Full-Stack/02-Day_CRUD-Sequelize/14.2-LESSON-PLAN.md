...
- Be sure to look at the solutions to each activity before the lecture begins, and to run the server files in each directory to see how they should run.
...
### Warm Up: Connect to Sequelize and Create a Todo Model (30 min)
...
- INSTRUCTIONS:
  ...
  6. Step 5 should have created a config and a models directory for us. Navigate to the config folder, open `config.json`, and modify the development object's "database","username" and "password" values to match your MYSQL database on your machine.
...
### Everyone Do: Review Update and Delete
...
### Partners: Sequelize Validations

- (15 min)

- Slack out the following directory in `Activities/10-Sequelize-Validations`
...
### Instructor: Review Validations and Dismiss Class
